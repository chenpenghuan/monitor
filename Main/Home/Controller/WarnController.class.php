...
class WarnController extends Controller {
	...
	public function savecreate() {
		...
		try {
			if ($_POST['warn_type'] && $_POST['warn_level'] && $_POST['warn_key'] && $_POST['warn_value'] && $_POST['warn_logic'] && $_POST['warn_send']) {
				if (!empty($_POST['warn_id'])) {	//报警id不为空，则更新报警设置
	...
	public function readcols() {
		...
		//var_dump($warn_cols);
		$wcs = null;
		$tbhead = '<div id="delmsg"><table  class="table table-bordered table-hover definewidth m10" style="font-size:12px;"><thead><tr><th>报警ID</th><th>字段id</th><th>报警类型</th><th>报警级别</th><th>报警参数</th><th>报警阀值</th><th>关系逻辑</th><th>报警对象</th><th>管理/<button onclick=create_cols_warn()>新建</button></th></tr></thead>';
		$trs = '';
		foreach ($warn_cols as $k => $v) {
			$trs .= '<tr><td>' . $v['id'] . '</td><td>' . $v['warn_colid'] . '</td><td>' . $v['warn_type'] . '</td><td>' . $v['warn_level'] . '</td><td>' . $v['warn_key'] . '</td><td>' . $v['warn_value'] . '</td><td>' . $v['warn_logic'] . '</td><td>' . $v['warn_send'] . '</td><td><button onclick=foritems2(all="act=edit&wk=' . $k . '",url="' . C('MOD_WARN') . '/edit_warn_cols",outid="create_warn",warn="N",warnword="确定提交吗？",add="数据库错误，请检查添加信息！")>修改</button><button onclick=foritems2(all="act=del&id=' . $v['id'] . '",url="' . C('MOD_WARN') . '/del_warn_cols",outid="create_warn",warn="Y",warnword="确定提交吗？",add="数据库错误，请检查添加信息！")>删除</button></td></tr>';
		}
		$tbfoot = '</table></div><div id="create_warn"></div>';
		echo $tbhead . $trs . $tbfoot;
		exit();
	}
	public function logic_text($logic) {
		switch ($logic) {
		case '4':
			return '<select class="form-control" id="warn_logic"><option value=4 selected="selected">无条件报警</option><option value=1>按大于匹配</option><option value=2>按小于匹配</option><option value=3>按包含匹配</option></select>';
			break;
		case '1':
			return '<select class="form-control" id="warn_logic"><option value=4 selected="selected">无条件报警</option><option value=1 selected="selected">按大于匹配</option><option value=2>按小于匹配</option><option value=3>按包含匹配</option></select>';
			break;
		case '2':
			return '<select class="form-control" id="warn_logic"><option value=4 selected="selected">无条件报警</option><option value=1>按大于匹配</option><option value=2 selected="selected">按小于匹配</option><option value=3>按包含匹配</option></select>';
			break;
		case '3':
			return '<select class="form-control" id="warn_logic"><option value=4 selected="selected">无条件报警</option><option value=1>按大于匹配</option><option value=2>按小于匹配</option><option value=3 selected="selected">按包含匹配</option></select>';
			break;
		default:
			return '<select class="form-control" id="warn_logic"><option value=4 selected="selected">无条件报警</option><option value=1>按大于匹配</option><option value=2>按小于匹配</option><option value=3>按包含匹配</option></select>';
			break;
	...
	public function edit_warn_cols() {
		...
		$form = '<form class="form-horizontal" role="form" style="padding-left:10%;padding-top:3%;width:80%;"><div class="form-group"><label for="firstname" class="col-sm-2 control-label">报警ID</label><div class="col-sm-10"><input id="id" type="text" class="form-control" readonly="true" value="' . $warn_conf[$_POST['wk']]['id'] . '"></div></div><div class="form-group"><label for="firstname" class="col-sm-2 control-label">字段ID</label><div class="col-sm-10"><input id="colid" type="text" class="form-control" value="' . $warn_conf[$_POST['wk']]['warn_colid'] . '"></div></div><div class="form-group"><label for="firstname" class="col-sm-2 control-label">报警类型</label><div class="col-sm-10"><input id="warn_type" type="text" class="form-control" value="' . $warn_conf[$_POST['wk']]['warn_type'] . '"></div></div><div class="form-group"><label for="lastname" class="col-sm-2 control-label">报警级别</label><div class="col-sm-10"><input type="text" class="form-control" id="warn_level" value="' . $warn_conf[$_POST['wk']]['warn_level'] . '"></div></div><div class="form-group"><label for="lastname" class="col-sm-2 control-label">报警参数</label><div class="col-sm-10"><input id="warn_key" type="text" class="form-control" value=' . $warn_conf[$_POST['wk']]['warn_key'] . '></div></div><div class="form-group"><label for="lastname" class="col-sm-2 control-label">报警阀值</label><div class="col-sm-10"><input id="warn_value" type="text" class="form-control" value=' . $warn_conf[$_POST['wk']]['warn_value'] . '></div></div><div class="form-group"><label for="lastname" class="col-sm-2 control-label">关系逻辑</label><div class="col-sm-10">' . $this->logic_text($warn_conf[$_POST['wk']]['warn_logic']) . '</div></div><div class="form-group"><label for="lastname" class="col-sm-2 control-label">报警对象</label><div class="col-sm-10"><input id="warn_send" type="text" class="form-control" value=' . $warn_conf[$_POST['wk']]['warn_send'] . '></div></div><div class="form-group"><div class="col-sm-offset-2 col-sm-10"><button onclick=foritems2(all="id="+document.getElementById("id").value+"&colid="+document.getElementById("colid").value+"&warn_type="+document.getElementById("warn_type").value+"&warn_level="+document.getElementById("warn_level").value+"&warn_key="+document.getElementById("warn_key").value+"&warn_value="+document.getElementById("warn_value").value+"&warn_logic="+document.getElementById("warn_logic").value+"&warn_send="+document.getElementById("warn_send").value,url="' . C('MOD_WARN') . '/savecreatecols",outid="edit_warn_info",warn="Y",warnword="确定提交吗？",add="数据库错误，请检查添加信息！") type="button" class="btn btn-default">提交</button><button type="button" class="btn btn-default" onclick=document.getElementById("create_warn").innerHTML="" >返回</button></div></div></form><div id="edit_warn_info" style="padding-left:15%;padding-top:3%;width:70%;color:#ff0000;"></div>';
		echo $form;
		exit();
	}
	...
	public function savecreatecols() {
		try {
			if (empty($_POST['colid']) || empty($_POST['warn_type']) || empty($_POST['warn_level']) || empty($_POST['warn_key']) || empty($_POST['warn_value']) || empty($_POST['warn_logic']) || empty($_POST['warn_send'])) {
				echo '请不要输入空内容';
				exit();
			}
			...
			if (!empty($_POST['id'])) {
				//如果存在id，即视为要修改配置
				$sql = 'update warn_cols_conf set warn_colid=' . $_POST['id'] . ',warn_type=\'' . $_POST['warn_type'] . '\',warn_level=' . $_POST['warn_level'] . ',warn_key=\'' . $_POST['warn_key'] . '\',warn_value=\'' . $_POST['warn_value'] . '\',warn_logic=' . $_POST['warn_logic'] . ',warn_send=\'' . $_POST['warn_send'] . '\' where id=' . $_POST['id'];
				file_put_contents(C('WARN_CONF'), '');
				$result = M()->execute($sql);
				if ($result) {
					echo '修改成功，<button type="button" class="btn btn-success" onclick="location.reload()">刷新</button>后即可显示！';
				...
				//如果不存在id，即视为要新建配置
				$sql = 'insert into warn_cols_conf(warn_colid,warn_type,warn_level,warn_key,warn_value,warn_logic,warn_send) values(' . $_POST['colid'] . ',"' . $_POST['warn_type'] . '",' . $_POST['warn_level'] . ',"' . $_POST['warn_key'] . '","' . $_POST['warn_value'] . '",' . $_POST['warn_logic'] . ',"' . $_POST['warn_send'] . '")';
				file_put_contents(C('WARN_CONF'), '');
				$result = M()->execute($sql);
				if ($result) {
					echo '添加成功，<button type="button" class="btn btn-success" onclick="location.reload()">刷新</button>后即可显示！';
