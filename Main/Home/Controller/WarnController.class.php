...
class WarnController extends Controller {
	...
	public function readcols() {
		...
		//var_dump($warn_cols);
		$wcs = null;
		$tbhead = '<div id="delmsg"><table  class="table table-bordered table-hover definewidth m10" style="font-size:12px;"><thead><tr><th>报警ID</th><th>字段id</th><th>报警类型</th><th>报警级别</th><th>报警阀值</th><th>关系逻辑</th><th>报警对象</th><th>管理/<button onclick=create_cols_warn()>新建</button></th></tr></thead>';
		$trs = '';
		foreach ($warn_cols as $k => $v) {
			$trs .= '<tr><td>' . $v['id'] . '</td><td>' . $v['warn_colid'] . '</td><td>' . $v['warn_type'] . '</td><td>' . $v['warn_level'] . '</td><td>' . $v['warn_value'] . '</td><td>' . $v['warn_logic'] . '</td><td>' . $v['warn_send'] . '</td><td><button onclick=foritems2(all="act=edit&wk=' . $k . '",url="' . C('MOD_WARN') . '/edit_warn_cols",outid="create_warn",warn="N",warnword="确定提交吗？",add="数据库错误，请检查添加信息！")>修改</button><button onclick=foritems2(all="act=del&id=' . $v['id'] . '",url="' . C('MOD_WARN') . '/del_warn_cols",outid="create_warn",warn="Y",warnword="确定提交吗？",add="数据库错误，请检查添加信息！")>删除</button></td></tr>';
		}
		$tbfoot = '</table></div><div id="create_warn"></div>';
		echo $tbhead . $trs . $tbfoot;
		exit();
	...
	public function savecreatecols() {
		try {
			if (empty($_POST['colid']) || empty($_POST['warn_type']) || empty($_POST['warn_level']) || empty($_POST['warn_value']) || empty($_POST['warn_logic']) || empty($_POST['warn_send'])) {
				echo '请不要输入空内容';
				exit();
			}
			...
			if (!empty($_POST['id'])) {
				//如果存在id，即视为要修改配置
				$sql = 'update warn_cols_conf set warn_colid=' . $_POST['id'] . ',warn_type=\'' . $_POST['warn_type'] . '\',warn_level=' . $_POST['warn_level'] . ',warn_value=\'' . $_POST['warn_value'] . '\',warn_logic=' . $_POST['warn_logic'] . ',warn_send=\'' . $_POST['warn_send'] . '\' where id=' . $_POST['id'];
				file_put_contents(C('WARN_COLS_CONF'), '');
				$result = M()->execute($sql);
				if ($result) {
					echo '修改成功，<button type="button" class="btn btn-success" onclick="location.reload()">刷新</button>后即可显示！';
				...
				//如果不存在id，即视为要新建配置
				$sql = 'insert into warn_cols_conf(warn_colid,warn_type,warn_level,warn_value,warn_logic,warn_send) values(' . $_POST['colid'] . ',"' . $_POST['warn_type'] . '",' . $_POST['warn_level'] . ',"' . $_POST['warn_value'] . '",' . $_POST['warn_logic'] . ',"' . $_POST['warn_send'] . '")';
				file_put_contents(C('WARN_COLS_CONF'), '');
				$result = M()->execute($sql);
				if ($result) {
					echo '添加成功，<button type="button" class="btn btn-success" onclick="location.reload()">刷新</button>后即可显示！';
