...
class formatdata(object):
    ...
    def __init__(self, ip, port, password, subcha,url,logfile,dbhost,dbuser,dbpass,dbname,dbport,dbcharset,confsfile):
        self.ip = ip
        self.port = port
            ...
            if msg['type'] == 'message':    #这里开始插件各自的自定义操作
                ...
                try:
                    ...
                    if confs != '':
                        ...
                        warn_cols_conf=self.readdata('select warn_colid,warn_type,warn_value,warn_logic,warn_center_id from warn_cols_conf')     # 查询出所有的已配置的统计报警信息
                        ...
                        confsfile.close()
                    data_collected=self.readdata('select contents.id,contents.cont_id,contents.cont_text,cont_conf.cont_title from contents,cont_conf where isshow=1 and contents.cont_id=cont_conf.id')
...
if __name__ == "__main__":
    ...
    obj = formatdata('192.168.1.154', 6379, '123123', subcha,'http://localhost/warn_test.php','/home/cph/jsons/plugin1.log','192.168.1.154','root','123123','winnerlook',3306,'utf8','/home/cph/jsons/warn_cols_config.json')
