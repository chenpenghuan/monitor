...
class Recieve(object):
    ...
    def writesql(self,sql):
        try:
            mydb=connect(host=self.dbhost,user=self.dbuser,password=self.dbpassword,db=self.dbname,port=self.dbport,charset=self.dbcharset)
            cur=mydb.cursor()
            cur.execute(sql)
            mydb.commit()
            cur.close()
    ...
    def readconf(self):
        ...
        if path.isfile(self.configfile):
            try:
                ...
                result=False
        if result==False:
            conn=connect(host=self.dbhost,user=self.dbuser,password=self.dbpassword,db=self.dbname,port=self.dbport,charset=self.dbcharset)
            cur = conn.cursor()
            count = cur.execute('select id,warn_type,warn_level,warn_key,warn_value,warn_logic,warn_send from warn_conf')
            result = cur.fetchall()
            cur.close()
            conn.close()
            dic = {}
