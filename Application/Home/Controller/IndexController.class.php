...
class IndexController extends Controller {
	...
	public function foritems2() {
		...
		if ($_POST['item2id'] == 6) {
			switch ($_POST['act']) {
			case 'list':
				$tbhead = '<div id="delmsg"><table  class="table table-bordered table-hover definewidth m10" style="font-size:12px;"><thead><tr><th style="width:9%">一级菜单/序号</th><th style="width:9%">二级菜单/序号</th><th style="text-align:center">数据来源</th><th style="width:3.5%">状态</th><th  style="width:8%">管理/<button onclick=foritems2("item2id=6&act=add") type="button" class="btn btn-xs">新建</button></th></tr></thead>';
			...
			case 'edititem':
				$str_form = '<form class="form-horizontal" role="form" style="padding-left:10%;padding-top:3%;width:80%;">
                            ...
                            <div class="form-group">
                                <label for="lastname" class="col-sm-2 control-label">数据来源</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="itm_from" placeholder="' . $this->dat[$_POST['item_id']] . '">
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-sm-offset-2 col-sm-10">
			...
			$title = M()->query('select id,cont_sec,cont_title,cont_var from cont_conf where item_id=' . $item_id . ' order by cont_sec');
			$title_c = count($title);
			$tb_head = '';
			for ($i = 0; $i < $title_c; $i++) {
				$tb_head .= '<th style="text-align:center">' . $title[$i]['cont_title'] . '</th>';
			}
			$data = M()->query('select * from item_' . $item_type . '_' . $item_id);
			$tb_body = '';
			foreach ($data as $k => $v) {
				$tb_body .= '<tr>';
				foreach ($v as $k => $v) {
